# 🏀 Timeout IQ: NBA Timeout Optimization Intelligence

**Timeout IQ** is an AI-powered analytics platform built to solve one of the most debated questions in basketball: *Does calling a timeout actually help?*
...
### 1. The Morale Model (Momentum Shock Index)
This model calculates a "Momentum Shock Index" (MSI) for every play. It isn't just looking at the score; it tracks:
...
This is a cold, hard machine learning classifier. We trained an **XGBoost** model on historical timeout data, labeling a timeout as "beneficial" if the team's performance (score differential or opponent run suppression) improved in the following 15 plays.
